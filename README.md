...
If you see an execution policy error, run `Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser` once, then retry.
...
```bash
sudo apt install nvidia-cuda-toolkit       # CUDA Toolkit (nvcc)
sudo apt install nsight-systems            # Nsight Systems (nsys)
# Nsight Compute (ncu) is bundled with CUDA in /usr/local/cuda/bin,
# or download from https://developer.nvidia.com/nsight-compute
```

Add CUDA bin to PATH if `ncu` or `nvcc` isn't found:
...
Colonel finds `nvcc` and `nsys` in their default locations. Nsight Compute needs to be added to PATH:
...
If your version differs, find the `ncu.exe` folder:
...
If `nsys` is missing, install from [Nsight Systems](https://developer.nvidia.com/nsight-systems) and add its bin folder to PATH.
